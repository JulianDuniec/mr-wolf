...
module.exports = {
	...
	*/
	start : function(params) {
		...
		this.onReEnqueuePeriodical = params.onReEnqueuePeriodical;
		this.loop();
	},
	...
	loop : function() {
		...
		if(this.running == true) {
			if(!this.lastCleanup) {
				...
				loopNext();
			} else if(new Date().getTime() - this.lastCleanup > this.cleanupInterval) {
				this.cleanup(function() {
					loopNext();
					me.lastCleanup = new Date().getTime();
				});
			} else {
	...
	*/
	...
	setCleanupInterval : function(cleanupInterval) {
	...
	*/
	...
	cleanup : function(cb) {
		this.db.collection('messages', function(err, collection) {
			collection.remove({finished : {$ne : null}}, function() {
	...
	reEnqueuePeriodical : function(message, cb) {
		this.db.collection('messages', function(err, collection) {
			var startAtDate = new Date();
			startAtDate.setSeconds(startAtDate.getSeconds()+message.meta.periodicalSeconds);
			collection.update({_id : message._id}, {$set : {startAtDate : startAtDate, started : null}}, null, cb);
		});
	},
		...
		try {
			...
			runner.receive(message.meta, function(err) {
				if(message.meta.periodicalSeconds) {
					me.reEnqueuePeriodical(message, function() {
						if(me.onReEnqueuePeriodical)
							me.onReEnqueuePeriodical(message);
						cb();
					});
				}
				else if(err) {
